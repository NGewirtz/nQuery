  ...
  $n('#jacket-button').on('click', () => {
    ...
    .then(data => {
      if (data.main.temp < 60) {
        $n('#todo-list').append(`<li>Bring A Jacket! (The current temperature in New York is below 60 degrees! Information courtesy of openweathermap API)</li>`);
      } else {
        alert("You Don't Need A Jacket (The current temperature in New York is above 60 degrees! Information courtesy of openweathermap API)");
      }
    });
  });
